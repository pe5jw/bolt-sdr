...
if ($PlanOnly) {
    [ordered]@{
        ...
        endpoints = $endpoints
        example = "powershell -NoProfile -ExecutionPolicy Bypass -File tools\capture-dsp-modernization-bundle.ps1 -BaseUrl $base -Label g2-candidate-before"
        desktopExample = "powershell -NoProfile -ExecutionPolicy Bypass -File tools\capture-dsp-modernization-bundle.ps1 -BaseUrl https://localhost:6443 -SkipCertificateCheck -Label g2-candidate-before"
...
3. Audit packaged WDSP runtime artifacts and side-by-side native dependencies:
   powershell -NoProfile -ExecutionPolicy Bypass -File tools\audit-wdsp-runtime-artifacts.ps1 -ReportPath "$bundleDir\artifacts\wdsp-runtime-artifact-audit.json" -FailOnMissingWinX64CurrentNr
...
6. Before the heavier live matrix, hunt for a G2 mixed weak+strong candidate DSP RX window from current frontend peaks:
   powershell -NoProfile -ExecutionPolicy Bypass -File tools\run-dsp-g2-rx-peak-hunt.ps1 -BaseUrl $base$certArg -OutputRoot "$bundleDir\artifacts\g2-rx-peak-hunt" -ReportPath "$bundleDir\artifacts\g2-rx-peak-hunt-report.json" -AllowRetune -StopOnReady -SamplesPerWindow 24 -IntervalMs 250 -MaxPeaks 6
   Omit -AllowRetune for a current-VFO-only dry run that lists candidate frontend peaks without moving the RX VFO.
7. Capture optional live runtime trends during scenario windows:
...
8. Or capture repeatable off, Thetis-parity, current-Zeus, and candidate DSP multi-scenario live diagnostics matrices:
   ...
   powershell -NoProfile -ExecutionPolicy Bypass -File tools\run-dsp-live-diagnostics-matrix.ps1 -BaseUrl $base$certArg -BundleDir "$bundleDir" -ComparisonId candidate-under-test -IndexPath "$bundleDir\artifacts\live-diagnostics-trace-index.candidate.json" -ReportPath "$bundleDir\artifacts\live-diagnostics-matrix-report.candidate.json" -Samples 60 -IntervalMs 1000
9. Summarize live tuning history after several Candidate/NR2 attempts; rerun this after the watcher summaries and JSONL traces are finalized because schema v9 records per-trace SHA-256 provenance:
...
10. Compare a candidate live trace against a baseline trace before accepting the window:
...
11. Compare candidate matrix windows against baseline matrix windows before accepting the scenario set:
...
12. For acceptance review, regenerate artifact-manifest.json with the required live evidence set after the comparison report is captured:
   powershell -NoProfile -ExecutionPolicy Bypass -File tools\new-dsp-artifact-manifest.ps1 -BundleDir "$bundleDir" -AcceptanceManifest -RequireLiveAcceptanceArtifacts -Force
13. Fill the required files listed in artifact-manifest.template.json.
14. Copy or regenerate the scaffold as artifact-manifest.json, then validate with:
...
15. Summarize validation/provenance failures, evidence-gate readiness, acceptance stage readiness, and the next evidence actions for review or CI triage:
