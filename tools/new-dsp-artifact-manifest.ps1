...
$output = [ordered]@{
    ...
    notes = @(
        ...
        "Use watch-dsp-manual-tune-observer.ps1 with -BundleDir, -MaxCapturesPerVfo 2, and -AllowStaleSceneCapture while the operator is manually tuning active G2 frequencies; the optional report is read-only/no-retune evidence that can retry an intermittent same-VFO window until mixed weak+strong evidence appears.",
