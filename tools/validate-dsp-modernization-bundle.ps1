        ...
        foreach ($artifact in $declaredArtifacts) {
            ...
            if ($artifactKind -ieq "metrics-json" -or $artifactId -eq "offline-fixture-metrics") {
                if ($artifactId -eq "offline-fixture-metrics") {
                    ...
                    if (-not $sourceWdspBacked) {
                        ...
                        Add-ArtifactIssue $errors $warnings -Required:$effectiveRequired "offline-fixture-metrics-not-wdsp-backed" "Artifact '$artifactId' must be generated by run-dsp-wdsp-fixture-evidence.ps1; evidenceEngine='$engineText'."
            ...
            if ($artifactKind -ieq "comparison-json" -or $artifactId -eq $metricComparisonArtifactId) {
                ...
                if (-not $wdspBackedEvidence) {
                    ...
                    Add-ArtifactIssue $errors $warnings -Required:$effectiveRequired "metric-comparison-not-wdsp-backed" "Artifact '$artifactId' must compare WDSP-backed fixture evidence from run-dsp-wdsp-fixture-evidence.ps1; evidenceEngine='$engineText'."
                    $artifactValidationOk = $false
                }
...
if ((Test-Truthy $offlineFixtureMetricsEvidence.present) -and (Test-Truthy $metricComparisonEvidence.present)) {
    ...
    elseif (-not [string]::Equals($expectedMetricsSha256, $actualMetricsSha256, [StringComparison]::OrdinalIgnoreCase)) {
        ...
        Add-AcceptanceIssue $errors $warnings -AllowPreflight:$AllowPreflight "metric-comparison-source-metrics-hash-mismatch" "Fixture metric comparison metricsSha256 '$actualMetricsSha256' does not match current offline-fixture-metrics SHA-256 '$expectedMetricsSha256'. Rerun compare-dsp-fixture-metrics.ps1 after regenerating fixture evidence."
        $metricComparisonEvidence["readyForReview"] = $false
        $metricComparisonEvidence["status"] = "not-ready"
    }
    else {
