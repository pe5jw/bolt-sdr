//
//  External ATU Control for Hermes-Lite v2 and SQUARE SDR 2
//    ATU Type: ICOM AH-4
//
...
//
//   WiSER, OK1XTG, Feb 26,2026
//   Fix - after successful tuning is completed, when KEY is released -> terminate carrier
...
logic key_seen = 1'b0; // WiSER - SQUARE SDR2

always @(posedge clk) begin // WiSER - SQUARE SDR2
  if (!enable) begin
    key_seen <= 1'b0;
  end else if (millisec_pulse) begin
    // KEY is considered active LOW in this module (see your FSM)
    if (~key) key_seen <= 1'b1;
  end
end
...
always @* begin
  ...
  case (state)
    ...
    PASS: begin
      // if we really saw KEY active during tuning, then after completion
      // hold inhibition so that TUN carrier does not run further
		// WiSER - SQUARE SDR2
      if (key_seen) txinhibit = 1'b1;
