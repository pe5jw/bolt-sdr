...
reg [3:0]spi_state;
reg [5:0]data_count;
reg [47:0]previous_Alex_data;	// used to detect change in data
reg loop_count;					// single bit used to send data word twice ech time data word has changed
...
0:	begin
		if (reset | Alex_data != previous_Alex_data)begin
			data_count <= 47;							// set starting bit count to 47
			spi_state <= 1;
		end
...
4:	begin
		if (data_count == 32)begin		// transfer complete for Tx data
