...
module Bootloader (
			  STATUS_LED, PHY_TX,PHY_RX,PHY_DV,PHY_TX_CLOCK,PHY_TX_EN, NCONFIG, SW17, SW18, EPCS_flash,
...
// image select 
input   wire     SW17;						//bootloader mode switch; high = normal, low = bootloader mode 
input   wire     SW18;					   //daughterboard bootloader mode switch/jumper, pin 25 on J43; OR'd with SW17

// flash part select
input   wire     EPCS_flash;			// J14 jumper off (high) = EPCS128, jumper on (low) = EPCQ128A
...
tx_pllv PLL_inst (.rst(), .refclk(C125_clk), .outclk_0(Tx_clock), .outclk_1(Tx_clock_2), .outclk_2(PHY_MDIO_clk), .locked(C125_locked));
...
assign MODE1 = SW17; // high -> normal mode, low -> bootoader mode

wire done;
wire CRC_error;

Reconfigure Recon_inst(.reset(reset), .clock(clock), .clk_freq(CLOCK_FREQ),.BootAddress(24'h200000),
					   .control(MODE1), .epcs(EPCS_flash), .CRC_error(CRC_error), .done(done)); 
...
0:	begin 
		...
		erase_done <= 0;
		sector_erase <= 0;
		address <= 24'h200000;		// set starting erase/program address to sector 4 (start of top 2MB) 
		byte_count <= 0;
		if (erase) 
			 state <= 1'd1;
...
2:	begin
		...
		sector_erase <= 0;
		if (busy) state <= 2;
		else if (address != 24'h600000) begin 	// reached end of memory yet?
...
ASMI_CV  ASMI_inst(.addr(address),.bulk_erase(0), .busy(busy), .clkin(Tx_clock), .data_valid(data_valid),
								 .dataout(dataout), .illegal_erase(error), .rden(read_enable), .fast_read(read),
								 .wren(write_enable), .sector_protect(1'b0),
								 .datain(datain), .sector_erase(sector_erase), .write(write), .shift_bytes(shift_bytes)); 
...
assign DEBUG_LED1 = ~IP_ready;
assign DEBUG_LED2 = 1'b1;
assign DEBUG_LED3 = ~EEPROM_IP[7];
assign DEBUG_LED4 = ~EEPROM_IP[6];
assign DEBUG_LED5 = ~EEPROM_IP[5];
assign DEBUG_LED6 = ~EEPROM_IP[4];
assign DEBUG_LED7 = ~EEPROM_IP[3];
assign DEBUG_LED8 = ~EEPROM_IP[2];
assign DEBUG_LED9 = ~EEPROM_IP[1];
assign DEBUG_LED10 = ~EEPROM_IP[0];
...
//assign DEBUG_LED10 = ~HB_counter[25]; 					// turn on bottom LED when busy  
