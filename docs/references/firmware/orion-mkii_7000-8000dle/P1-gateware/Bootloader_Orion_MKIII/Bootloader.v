...
2:	begin
		...
		sector_erase <= 0;
		if (busy) state <= 2;
		else if (address != 24'h400000) begin //h1F0000) begin	// reached end of memory yet?
			//if (EPCS_flash) address <= address + 24'h040000;	// increment address to next sector
			//else address <= address + 24'h010000;	// increment address to next sector
			address <= address + ((EPCS_flash) ? 24'h040000 : 24'h010000);	// increment address to next sector
			state <= 1;
		end 		
...
ASMI2  ASMI_inst(.addr(address),.bulk_erase(0), .busy(busy), .clkin(Tx_clock), .data_valid(data_valid),
