...
2022		 Nov 8	- (N1GP) migrated from Cyclone IV to cyclone V
						- changed firmware version number to v2.8
...
module Orion(
  ...
  input  MODE2,                  //jumper J14 on Orion: 1 if removed; 0 if jumpered (not currently used)
  input  ANT_TUNE,               //atu
...
parameter  Orion_version = 8'd28;		// Serial number of this version
...
PLL_IFV PLL_IF_inst (.rst(1'b0), .refclk(C122_clk), .outclk_0(DACD_clock), .outclk_1(CMCLK), .outclk_2(CBCLK),  .outclk_4(CLRCLK), .locked(IF_locked));

//PLL_30MHz PLL_30MHz_inst (.rst(1'b0), .refclk(_122MHz), .outclk_0(userADC_clk)); // generates 30.720 MHz clock for Orion_ADC module
...
PLL_clocks PLL_clocks_inst(.rst(1'b0), .refclk(C125_clk), .outclk_0(EEPROM_clock), .outclk_1(Tx_clock), .outclk_2(Tx_clock_2), .locked(C125_locked));
...
Attenuator Attenuator_ADC1 (.clk(CLRCLK), .data(attenuator1), .ATTN_CLK(ATTN_CLK), .ATTN_DATA(ATTN_DATA), .ATTN_LE(ATTN_LE));
Attenuator Attenuator_ADC2 (.clk(CLRCLK), .data(attenuator2), .ATTN_CLK(ATTN_CLK_2), .ATTN_DATA(ATTN_DATA_2), .ATTN_LE(ATTN_LE_2));
...
C122_PLL PLL_inst (.rst(1'b0), .refclk(_122MHz), .outclk_0(osc_10MHz), .outclk_1(IF_clk), .locked());	

// generate a phase shifted 122.88MHz clock for the TxDAC (DACD) data
//C122_SHIFT_PLL SHIFT_inst(.rst(1'b0), .refclk(_122MHz),.outclk_0(DACD_clock), .locked());
