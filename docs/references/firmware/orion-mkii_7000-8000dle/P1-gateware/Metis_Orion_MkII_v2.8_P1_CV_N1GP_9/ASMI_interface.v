...
reg 	[23:0]address; 		// address in EPCQ128A to write to
reg 	write_enable; 
reg 	write;
reg     shift_bytes; 
reg 	[3:0]state;
...
0:	begin 
		...
		erase_ACK <= 0;
		erase_done <= 0;
		address <= 24'h200000;			// set starting address to write to is top 2MB
		byte_count <= 0;
		page <= 0;
		send_more <= 0;
		NCONFIG <= 0;
		if (erase) 
			 state <= 1'd1;
		else if(IF_Rx_used > 254) 		// if we have enough data in the Rx fifo then send to the EPCQ128A
			state <= 5;		 
	end
...
// program EPCQ128A with data from PC
5: 	begin 
	rdreq <= 1'b1;					
...
ASMI_CV  ASMI_inst(.addr(address),.sector_erase(sector_erase), .busy(busy), .clkin(clock),
