...
2021	Sep 1 - (N1GP) Added autonegotiated 100T/1000T support, merged from the HL folks.
                       ...
                       (This has been removed as it would not pass timing for many Orion2 boards).

2022	Apr 21 - (N1GP) Added a dedicated RX slice for the temp_DAC output. This is used for puresignal
...
adc_mag_fifo adc0_mag_fifo_inst(.wrclk (C122_clk),.wrreq (1'b1), .rdclk (tx_clock), .rdreq (adc0_ren), 
				.rdusedw(adc0_rused), .data (temp_ADC[3][0]), .q (ADC0_unsigned)); 
adc_mag_fifo adc1_mag_fifo_inst(.wrclk (C122_clk),.wrreq (1'b1), .rdclk (tx_clock), .rdreq (adc1_ren), 
				.rdusedw(adc1_rused), .data (temp_ADC[3][1]), .q (ADC1_unsigned)); 
...
CC_encoder CC_encoder_inst (
					...
					.Debug_data({13'd0, ~DEBUG_LED10, keyout, is_9031}),
					//.Debug_data(16'd0),
...
Attenuator Attenuator_ADC0 (.clk(CLRCLK), .data(atten0), .ATTN_CLK(ATTN_CLK),   .ATTN_DATA(ATTN_DATA),   .ATTN_LE(ATTN_LE));
Attenuator Attenuator_ADC1 (.clk(CLRCLK), .data(atten1), .ATTN_CLK(ATTN_CLK_2), .ATTN_DATA(ATTN_DATA_2), .ATTN_LE(ATTN_LE_2));
...
Led_flash Flash_LED10(.clock(rx_clock), .signal(discovery_reply), .LED(DEBUG_LED10), .period(fast_clock_half_second)); // note fast clock now
...
always @(posedge CLOCK_25MHZ) begin
	...
	Status_LED <= HB_counter[is_9031 ? 24 : 23]; // Blink slower for 9031
end
