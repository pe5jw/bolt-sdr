...
module Mercury(OSC_10MHZ, ext_10MHZ,AUX_CLK, C122_clk,INA,CC,ATTRLY,A6,A12,C19,C21,C22,C23,C24,MDOUT_I,MDOUT_Q,CDIN,
			   ...
			   DEBUG_LED6,DEBUG_LED7, Attn_Ctrl, Merc_ID,Merc_ID_drv, MULTIPLE_MERC, SDA, SCL); 
...
output wire  [4:0] Attn_Ctrl;      // GPIO 17 - 13
...
localparam SERIAL = 8'd41; // software version serial number
...
pulsegen reset_Alex  (.sig(C122_rst), .rst(0), .clk(BCLK), .pulse(SPI_Alex_rst));
...
reg [1:0] RxN_preamps;    // status of Mercury preamps (00=all preamps OFF, 01=Merc1 preamp ON, 10=Merc2 preamp ON)
...
begin
  ...
  begin
    ...
    attn0            <= C122_rcv_data[101:97]; // Attenuator control bits, active low
...
NWire_rcv  #(.OSL(102), .OSW(1), .ICLK_FREQ(122880000), .XCLK_FREQ(122880000), .SLOWEST_FREQ(10000)) 
...
cdc_sync #(1) cdc_sync_Alex_enable
   (.siga(C122_Alex_enable), .rstb(0), .clkb(SPI_clk), .sigb(SPI_Alex_enable));

SPI Alex_SPI_Tx (.reset(SPI_Alex_rst), .enable(SPI_Alex_enable), .Alex_data(SPI_Alex_data), .SPI_data(SPI_data),
...
assign DEBUG_LED3 = merc1_2_pres;
assign DEBUG_LED4 = adc2;
assign DEBUG_LED5 = (Merc_ID == 3'b000);
assign DEBUG_LED6 = (Merc_ID == 3'b001);
`endif
assign DEBUG_LED3 = Attn_Ctrl[0];
assign DEBUG_LED4 = Attn_Ctrl[1];
assign DEBUG_LED5 = Attn_Ctrl[2];
assign DEBUG_LED6 = Attn_Ctrl[3];
assign DEBUG_LED7 = Attn_Ctrl[4];
...
assign {DEBUG_LED1,DEBUG_LED2} = counter[24:23];  // even faster flash for this version!
