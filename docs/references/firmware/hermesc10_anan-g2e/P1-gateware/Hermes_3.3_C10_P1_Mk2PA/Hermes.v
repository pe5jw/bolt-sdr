...
/* 	This program interfaces the LTC2208 to a PC over Ethernet.
   ...
   15  Sep        - Set wide spectrum FIFO to 16k.  Enabled second receiver.
	...
	20 Mar 2017		- modified SPI.v to send Alex data word twice each time the data word changes
	 ...
	 2 Sep 2017    - Reduced gain in EER iFIR to prevent overload when full scale I&Q used for EER/ET
						  by modifying coefI5_1025_EER.mif.
						  Increased output level of EER envelope signal so as to provide full voltage range.						  
						- changed version number to v3.3
	24 Dec 2017    - Fixed problem where ET/EER data would sometimes not start.
...
module Hermes(
  ...
  input  ANT_TUNE,               //atu
  output DRV_BIAS,               //low to enable drive bias
...
initial DRV_BIAS = 1'b1;                             // ensure drive bias is OFF initially
assign  DRV_BIAS = ~FPGA_PTT;                        // drive bias switching, low turns drive bias ON during Tx
...
parameter  Hermes_serialno = 8'd33;		// Serial number of this version
...
assign EER_enable = (ramp == 10'd0 | ramp == 10'd1); // need an enable at clk_245/2 to enable EER FIR.
...
FirInterp5_1025_EER fiEER (_122MHz, EER_enable, req_EER, C122_EER_i, C122_EER_q, I_EER, Q_EER); 
...
begin 
  ...
  begin 										// Need to ensure that C&C data is stable 
    ...
    begin
	  ...
	  IF_Apollo         <= 1'b0;
	  //RRK IF_Apollo         <= IF_Rx_ctrl_2[5];      // 1 = Apollo enabled, 0 = Alex enabled 
...
// The following always block finds the receivers with the lowest RXn numbers that are assigned to the ADC0 and uses 
// the associated frequencies to determine the BPFs used in auto Alex switching mode for the ADC signal paths, respectively.
//
...
// 
always @ (posedge C122_clk) begin //coded for Hermes_C10 w/ Mk2PA
        if (C122_cbrise) begin
                C122_freq_temp <= C122_frequency_HZ[0];         // initial assignments for sorting
     
                if (C122_frequency_HZ[3] > 32'd0) C122_freq_temp <= C122_frequency_HZ[3];
                if (C122_frequency_HZ[2] > 32'd0) C122_freq_temp <= C122_frequency_HZ[2];
                if (C122_frequency_HZ[1] > 32'd0) C122_freq_temp <= C122_frequency_HZ[1];
                if (C122_frequency_HZ[0] > 32'd0) C122_freq_temp <= C122_frequency_HZ[0];
...
wire  [6:0] C122_select_HPF;
wire  [6:0] C122_select_HPF_auto;
...
// turn off 6m rx preamp during tx when Alex filters are under firmware control
...
assign C122_Alex_Rx_data = {1'b0, C122_RX_MASTER_IN_SEL, 1'b0, C122_HPF[5],
                       C122_Rx_1_in, 1'b0, C122_Rx_2_in, C122_Transverter, 1'b0,
                       C122_HPF[4:2], C122_6m_preamp, C122_HPF[1:0], 1'b0};

localparam  TX_YELLOW_LED = 1'b0;
assign C122_Tx_red_led = C122_TR_relay;
`else
// assign attenuators
...
cdc_sync #(32) SPI_Alex (.siga(C122_Alex_data), .rstb(SPI_Alex_rst), .clkb(CBCLK), .sigb(SPI_Alex_data));
