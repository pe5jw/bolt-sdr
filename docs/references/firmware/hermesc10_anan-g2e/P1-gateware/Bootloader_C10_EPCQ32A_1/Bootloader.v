...
0:	begin 
		...
		erase_done <= 0;
		sector_erase <= 0;
		address <= 24'h100000;		// set starting erase/program address to sector 4 (start of top 2MB) 
		byte_count <= 0;
		if (erase) 
			 state <= 1'd1;
...
2:	begin
		...
		sector_erase <= 0;
		if (busy) state <= 2;
		else if (address != 24'h1C0000) begin //h1F0000) begin	// reached end of memory yet?
