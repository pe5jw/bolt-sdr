...
reg [47:0] shift_Alex_data; // for 7000mk2 PA board
...
3'd0:	begin
		if (reset | ( enable & (Alex_data != previous_Alex_data))) begin
			...
			shift_Alex_data <= {Alex_data[31:16],16'b0,Alex_data[15:0]};
...
3'd1:	begin
		SPI_data <= shift_Alex_data[data_count];	// set up data to send
		spi_state <= 3'd2;
	end
3'd2:	begin
...
3'd4:	begin
		if (data_count == 6'd32) begin
			...
			Rx_load_strobe <= 1'b1;			// (pa7000) strobe the 32 bits of Rx1 and Rx0 data out in parallel
			spi_state <= 3'd6;
		end
		else begin
