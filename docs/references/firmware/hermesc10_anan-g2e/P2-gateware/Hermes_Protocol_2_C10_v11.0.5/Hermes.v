...
2020	Jan 4 - (N1GP) Fixed DHCP issue where a dhcp transaction not bound for the local MAC would get passed up and interfere
			 ...
			 when enabled.
			 - Changed FW version number to v10.8

2021	Jun 7 - (N1GP) Removed/fixed stuck mode in sdr_send.v, merged HL2 dhcp & icmp updates in.
                       Added beta_version for better tracking of test releases (byte 23 of discovery, per protocol2 doc v3.8).
...
2024	Jan 7 - (N1GP) Added use of Alex1's upper 16 bits for TX relay control. Namely the ANT1-3 bits.
...
2024    Oct 18 - (N1GP) Removed debounce_PTT from input conditions of FPGA_PTT assignment, to prevent possible
...
`ifdef HERMES_MK2PA
localparam board_type = 8'd20;		  	// 20 Hermes with MK2PA board
`else
localparam board_type = 8'd01;		  	// 00 for Metis, 01 for Hermes, 02 for ANAN-10E, 03 for Angelia, and 05 for Orion
`endif
parameter  Hermes_version = 8'd110;	// FPGA code version
parameter  beta_version = 8'd05;	// Should be 0 for official release
...
PLL_IF PLL_IF_inst (.inclk0(_122MHz), .c0(CMCLK), .c1(CBCLK), .c2(CLRCLK), .locked(IF_locked));
...
// This is just for Rx0 since it can sync with Rx1.
		...
		Rx_fifo_ctrl #(NR) Rx0_fifo_ctrl_inst( .reset(!C122_run || !C122_EnableRx0_7[0] ), .clock(C122_clk), .data_in_I(rx_I[NR]), .data_in_Q(rx_Q[NR]),
							.spd_rdy(strobe[0]), .spd_rdy2(strobe[NR]), .fifo_full(Rx_fifo_full[0]),
...
reg [15:0]temp_ADC;
reg [15:0] temp_DACD; // for pre-distortion / diversity
...
reg [15:0] ADC0_unsigned; // take ADC data AFTER the de-RANDOMIZER
reg [15:0] ADC0_mag;
reg [1:0] adc0_rused;
reg [1:0] adc_state = 0;
reg adc0_ren;
...
adc_mag_fifo adc0_mag_fifo_inst(.wrclk (C122_clk),.wrreq (1'b1), .rdclk (tx_clock), .rdreq (adc0_ren), 
				.rdusedw(adc0_rused), .data (temp_ADC), .q (ADC0_unsigned)); 
...
(.siga(Rx_frequency[1]), .rstb(C122_rst), .clkb(C122_clk), .sigb(C122_frequency_HZ[1]));	
	...
	.out_strobe(strobe[0]),
	//input
	.in_data(temp_ADC),  // to allow for PureSignal operations
	//output
	.out_data_I(rx_I[0]),
	.out_data_Q(rx_Q[0])
	);
	...
	//.sample_rate(C122_SyncRx[0][1] ? 16'd192 : C122_SampleRate[1]),
...
assign FPGA_PTT = IO4 && ((break_in && CW_PTT) || PC_PTT); // CW_PTT is used when internal CW is selected
...
wire [31:0]runsafe_Alex_data = {Alex_upper[15:12], run & PA_enable & (FPGA_PTT | Alex_upper[11]),
...
SPI Alex_SPI_Tx (.reset (SPI_Alex_rst), .enable(Alex_enable[0]), .Alex_data(SPI_Alex_data), .SPI_data(Alex_SPI_SDO),
                 .SPI_clock(Alex_SPI_SCK), .Tx_load_strobe(SPI_TX_LOAD), .Rx_load_strobe(SPI_RX_LOAD), .spi_clock(CBCLK));	
