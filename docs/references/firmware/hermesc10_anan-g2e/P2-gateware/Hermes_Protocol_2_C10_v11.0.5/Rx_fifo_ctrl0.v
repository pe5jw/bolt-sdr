...
if (reset) wrenable <= 0;	
	...
	2:	begin
			data_out <= {1'b0, Sync_data_in_I[15:8]};
			state <= 3;
		end		
		
	3:	begin
			data_out <= {1'b0, Sync_data_in_I[7:0]};
			state <= 4;
		end
	
	4:	begin
			data_out <= {1'b0, Sync_data_in_Q[23:16]};
			state <= 5;
		end

	5:	begin
			data_out <= {1'b0, Sync_data_in_Q[15:8]};
			state <= 6;
		end	
		
	6:	begin
			data_out <= {1'b0, Sync_data_in_Q[7:0]};
			state <= 7;
		end	
	...
	7: begin 
		 if (Sync == 1'b0) begin
			...
			data_out <= {1'b1, data_in_I[23:16]};
			state <= 8;
		 end 
		end 
		
	8:	begin
			data_out <= {1'b0, data_in_I[15:8]};
			state <= 9;
		end		
		
	9:	begin
			data_out <= {1'b0, data_in_I[7:0]};
			state <= 10;
		end
		
	10:	begin
			data_out <= {1'b0, data_in_Q[23:16]};
			state <= 11;
		end

	11:	begin
			data_out <= {1'b0, data_in_Q[15:8]};
			state <= 12;
		end	
		
	12:	begin
			data_out <= {1'b0, data_in_Q[7:0]};
			state <= 13;
		end
