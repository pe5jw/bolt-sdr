...
/*
	2015 Aug 13 - Start porting from Hermes, 2 Receivers.
					- Save as today's date
					- will not fit.
				   - Remove programmer code
					- Reduced Spectrum FIFO to 8k.
					- Removed wideband code.
					- Reduce Rx audio fifo from 4k to 1k.
					- Reduce Tx IQ fifo from 4k to 1k.
					- Error (170012): Fitter requires 1565 LABs to implement the design, but the device contains only 1539 LABs
					- Remove ping code.
					- Error (170012): Fitter requires 1557 LABs to implement the design, but the device contains only 1539 LABs
					- Remove code associated with ping.
					- Error (170012): Fitter requires 1542 LABs to implement the design, but the device contains only 1539 LABs
					- Remove all but test LEDs 1 & 2 & 4 & 7
					- Error (170012): Fitter requires 1546 LABs to implement the design, but the device contains only 1539 LABs
					- Remove LEDs 4 & 7
					- Error (170012): Fitter requires 1546 LABs to implement the design, but the device contains only 1539 LABs
					- Remove all LEDs
					- Fits - not meet timing (24506)
					
					
				14	- Use correct timing file! (24431) 99% - Works!
				   - Added LEDs 1 & 2 (24489) - Works!
					- Saved as today's date
					- Added Wideband - set Spectrum FIFO to 4k since run out of MK9 parts.
					- Error (170012): Fitter requires 1556 LABs to implement the design, but the device contains only 1539 LABs
				15 - Try Wideband with all settings fixed.					
					- Fix number of I&Q samples per frame when in Sync or Mux mode
					- Error (170012): Fitter requires 1550 LABs to implement the design, but the device contains only 1539 LABs
					- Does not run - saved as today's date.
					- Remove DHCP - Works.
					- Increased Rx and Tx_IQ fifos to 4k, note Tx_IQ fifo was 4k - Fails due to lack of RAM.
					- Reduce RX fifo to 2k. - Fails due to lack of RAM
					- Reduce Tx IQ fifo to 2k - Works but tune has a problem, try increasing fifo size.
					- Reduce Rx fifo to 1k and increase Tx to 4k - Tune OK now but 48ksps audio on KK has pops and Tx signal on 6m poor
					  also issue with 10MHz reference PLL. 
					- Try timing closure - min pulse width error on tx clock.
					- Redo timing from scratch - no improvement, revert to previous. 
					- **** could be problem in PSDR - wait for new version?
    				- revert to negative edge clocking - Works, 48k audio distortion on KK, occasional sequence error on Rx2. 
					- With new version of Thetis, Tx signal is clean on 6m and audio OK. Distorted 48k audio on KK. Lots of sequence errors on Rx2 when enabled.
					- Basically working.
					- Saved as today's date.
					
				16 - Rebuild all Megafunctions for Cyclone 3. Same but compile time seems to have been reduced to 8 mins
				   - Increase Rx fifo to 2k and reduce I&Q to 2k. 48ksps OK on KK and no Rx 2 errors on Thetis but CC errors.  Tx signal wobbles.
					- Increase I&Q to 4k and reduce wideband to 2k. Hermes drops data from PC at times.
					- Increase Rx audio fifo to 4k and reduce I&Q to 1k. Will not fit - not enough RAM
					- Set Rx audio fifo to 2k and wideband to 2k, Tx I&Q left at 4k. All seems to run OK on both KK and Thetis.
					- saved as today's date.
					- Only 3 PLLs being used instead of 4. Quartus is trying to merge the 122.88MHz to 80kHz PLL. TURN AUTO MERGE OFF !!
					- PLLs now all working. 
					- ADC3 overload occasionally flashing - investigate. Set ADC1_overload to 0.
					- ADC0 now flashing - saved as today's date_2.
				17 - Try initial clearing of C&C data & temp RAM.
				   - Removed ADC1_overload for C&C.
				   - Test - try removing ADC0. Sequence errors and distorted audio.	
				19 - Corrected C&C data to send ADC overloads correctly. OK
			      - Try adding data loaded LED.	OK
					- Saved as today's date.
				20 - Added one second time out if no C&C received, but not implemented
				27 - Updated Discovery reply to comply with openHPSDR Protocol V1.5
				   - Saved as today's date. 
				28 - Set wideband data to 2k per frame. 
				   - Removed for loop in sdr_send.
					- All appears to work - saved as today's date.
				29 - Released to Simon as Hermes_10E_29_Aug.rbf
				
		Sep    5 - Fixed Alex interface.
					- saved as today's date.
					- Redo timing.
					- saved as today's date.
					- Released to Simon as Hermes_10E_5_Sept.rbf 
				 6 - Added 10MHz PLL locked to CC_encoder
				   - Odd CC sequence error - redo timing.
					- change sdr_send to use for loop
				   - set ADC1_overload to 0 to prevent random values.	
					- Saved as today's date.
					- Released to Simon as Hermes_10E_6_Sept.rpf
		Oct    1 - Added Mux code for PureSignal operation.
		         - Removed reset code from receiver2.v
				 2	- redo timing. All works. 
				   - saved as today's date.
	   Nov    8 - Removed ICMP check from ip_recv which was changing send port when Destination unreachable ICMP packet received.
					- Removed (to_port == 16'd68) check in udp_recv since 1OE does not support ICMP.
					- Increased Rx gain by 12dB to match previous code. 
					- Fixed I&Q sequence. Swap I&Q out of FIR in receiver2.v
					- Move Open Collector outputs. In High Priority, was 1417:	Open_Collector  now 1401:
					- Remove DC offset in Tx CIC. 
					- Set Exciter, FWD and REV power to zero when FPGA_PTT not set. (see line 1325)
					- Set 16bit DAC feedback. NOT DONE
					- Corrected Rx input connected to Tx DAC for PureSignal. Now use number of ADCs.  
				13 - Enabled hardware timer. 
					- Moved I&Q sequence to previous. Swap I&Q out of FIR in receiver2.v
		Dec    4 - Fixed bug causing mic PTT to latch
               - Set protocol version to v2.2	
				 5 - Fixed Discovery reply packet length, now 50 bytes.
				   - Saved as today's date. 
				 6 - Enable Discovery to IP address. sdr_receive.v line 97.
				   - Redo timing. Saved as todays date and released.
				19 - Bring up to date with Hermes and Angelia
					  Set protocol version to V2.3
					  Added Hermes HW timer code.
					  Allow Discovery to any IP address
					  Added 16 bit DAC code from Hermes
					  Added new profile.v and added 'raised cosine profile.xls' to files. 
					- Save as today's date
				23 - Corrected mic and wideband by swap byte order
				   - Save as today's date and release.
				28 - Test code for Simon - add LEDs on I&Q FIFO full empty
					- Added I&Q FIFO_empty & FIFO_full signals to High Priority data
					- Sent to Simon for testing.
					- Changed FIFO_full to getting_full which is set with > 3800 samples in FIFO.
					- Sent to Simon for testing. Hermes_28_Dec(2).rbf.
					- stuffs Rx data sequence numbers when Txing.
				30 - Removed getting_full, just use empty and removed from LEDs. 
					- Fitter can't get it to fit.
				31 - try getting full and getting empty again.
				   - Sent to Simon for testing.
	2016 Jan  3 - Clear TR relay and Open Collectors if run not active, see lines 275, 1275, 1434
					- Sent to Simon for testing - 100% full!
					- saved as today's date.
				30 - Try corrections and new features
						hardware timer  >= and ++ 
						open collectors 1'b0
						...
						Fixed mixed blocking and non blocking errors vis:
							...
							sdr_send.v lines 135, Rx_sequence_number[j] ++
							profile.v line 102
							receiver2.v lines 70 - 75  (rate0)
						Fix sequence error in sdr_send and x++
						IO5 Tx enable input, uses iambic
						IO4 input for Tx inhibit when low (only inhibit DAC ouput)
						try user analogue input 1. OK if not gated with FPGA_PTT.
						Saved as today's date and released for testing.
		  Feb 22  - Added new FIR from Angelia
					 - Added HW_timer enable
					 - new HW_reset code
					 - Moved mux to Rx_specific_CC Byte 1363
					 - Won't fit, changed fitter to used Balanced. Using Byte 1363 seems to have caused fit problem
					 - With balanced intermittent audio 
					 - Use Hermes sdc file and re-do timing - OK
					 - Saved as today's date and released
					 - *** need to build again with protocol version = 2.7 
		  Aug 14  - Rebuilt for version = 2.9.
					 - Released to Beta testers. 
		  Sep  7  - Changed sdc file to PHY_TX[*]
					 -	Changed to short packets for mic and Rx audio
					 - FPGA code to V10.1, protocol to V3.0
				 8  - Re-do timing - OK.  Need set Process Priority to 'Above Normal' to prevent Sequence errors. 
					 - Released 
				24  - Modified WB data to only send if no DDC data available.
				    - If enable HW timer code stops. If disable then loose Rx audio at times. 
		  Oct 26  - Added Mux_clear code plus required changes to support from Angelia. 
	             - redo timing closure.	
		  Nov  3  - Try negative clock phase feeding DAC PureSignal feedback. 	 
				 7  - Mod when sdr_send ready, redo timing
				17  - Set DAC feedback directly to Rx1 input, PS works but not every time its selected.
						Feed Rx0 frequency and speed to Rx1. Works OK
						Use Mux bit to select Rx0 frequency and speed to Rx1. Redo timing. Works OK. 
				20  - Use Mux bit to select Rx1 input, either ADC or DAC. 
		      26  - Update Protocol version and code version.
						Redo timing.
						Release for testing.
		 Dec  18  - Enable CW sidetone when Rx audio FIFO empty.
						Release for testing.
				30  - Saved as today's date.
				31  - Set bits_per_sample in sdr_send.v in line since reporting 26 not 24. OK Now
					 - Saved as today's data, and released for testing.
		 Jan   2  - Simon reports high noise floor after a few seconds of operation (OK with Theits though). Appears to be only sending 
						1440 not 1444 bytes. He does not clear his Rx buffer before re-use so this could be why it does not work with 
						his software. 
						Try setting samples_frame and bits_per_sample as localparam.
		   			Works with Simon's code. Saved as today's date and release for testing.
		 Feb   6  - Simon reports that DDC data stops at times. Try adding addtional bit to Rx FIFO contents register so it does not
						indicate empty when actually full. 
					 - Saved as today's date and released to Simon for testing.
 2019  Apr  26  - Project file from Github 10E directory.  Doug reports port 1025 High Priority packets 
						(hardware to PC) have the wrong data size. The payload should be 60 bytes and firmware 10.2 is sending a payload of 64 bytes.
						Fixed error, changed sdr_send.v to send correct number of bytes.
					 - Updated release to v10.3
				27  - Added features from OrionII sdr_send.v that prevents hang plus default: & phy_ready.
...
module Hermes(
  ...
  //rx adc (LTC2208)
  input  [13:0]INA,              //samples from LTC2208
...
localparam board_type = 8'h02;		  	// 00 for Metis, 01 for Hermes, 02 for ANAN-10E, 03 for Angelia, and 05 for Orion
parameter  Hermes10E_version = 8'd103;	// FPGA code version
parameter  protocol_version = 8'd31;	// openHPSDR protocol version implemented
...
sdr_send #(board_type, NR, master_clock, protocol_version) sdr_send_inst(
	...
	.local_mac(local_mac),
	.code_version(Hermes10E_version),
	...
	//.samples_per_frame(16'd238),			// fixed as no sync or mux function
	//.tx_length(16'd1444),					// fixed as no sync or mux function
	.Wideband_packets_per_frame(8'd4),  // fixed for 512 by 16 bit samples per packet so 2k samples per frame
...
// This is just for Rx0 since it can Muxed with Rx1.
		...
		Rx_fifo_ctrl0 #(NR) Rx0_fifo_ctrl_inst( .reset(!run || !EnableRx0_7[0] ), .clock(C122_clk), .data_in_I(rx_I[1]), .data_in_Q(rx_Q[1]),
													.spd_rdy(strobe[0]), .spd_rdy2(strobe[1]), .fifo_full(Rx_fifo_full[0]), .Rx_fifo_empty(Rx_fifo_empty),  //.Rx_number(d),
													.wrenable(Rx_fifo_wreq[0]), .data_out(Rx_fifo_data[0]), .fifo_clear(Rx_fifo_clr[0]),
													.Sync_data_in_I(rx_I[0]), .Sync_data_in_Q(rx_Q[0]), .Sync(Mux), .convert_state(convert_state));	
...
// move flags into corrent clock domains
wire C122_phy_ready;

cdc_sync #(1) cdc_phyready  (.siga(phy_ready), .rstb(C122_rst), .clkb(C122_clk), .sigb(C122_phy_ready));

Mux_clear Mux_clear_inst( .clock(C122_clk), .Mux(Mux[1]), .phy_ready(C122_phy_ready), .convert_state(convert_state),
								  .fifo_clear(fifo_clear), .fifo_write_enable(write_enable), .fifo_empty(Rx_fifo_empty));		
generate
genvar d;
...
for (d = 1 ; d < NR; d++)
	begin:p
		...
		// If Sync[n] enabled then select the data from the receiver to be Muxed.
...
//------------------------------------------------
//   SP_fifo  (2k words) dual clock FIFO
//------------------------------------------------
...
// (default to 1024 bytes or 512 ADC samples) as is the sample size (defaults to 16 bits).
// The number of frames sent, per fifo fill, is set by the user - currently set at 4 i.e. 2k samples. 


wire have_sp_data;

wire wideband = Wideband_enable[0];  		// enable Wideband data if  selected

SP_fifo  SPF (.aclr(!wideband), .wrclk (C122_clk), .rdclk(tx_clock), 
             .wrreq (sp_fifo_wrreq), .data ({temp_ADC[7:0], temp_ADC[15:8]}), .rdreq (sp_fifo_rdreq),
...
//	if (wb_counter == (Wideband_update_rate * update_rate)) begin	  // max delay 255mS
	if (wb_counter == (100 * update_rate)) begin	  // *** fixed for ANAN-10E
		wb_counter <= 25'd0;
...
// send receiver audio to TLV320 in I2S format. Enable sidetone when no audio from PC.
audio_I2S audio_I2S_inst (.BCLK(CBCLK), .empty(Audio_empty && !CW_PTT), .LRCLK(CLRCLK), .data_in(Rx_audio), .data_out(CDIN), .get_data(get_audio_samples)); 
...
// Ensure I&Q data is zero if not transmitting or IO4 is low. 
wire [47:0] IQ_Tx_data = FPGA_PTT ? C122_IQ1_data : 48'b0; 													
													
// indicate how full or empty  the FIFO is 
wire almost_full 	= (write_used[11:8] == 4'b1111) ? 1'b1 : 1'b0;
wire almost_empty = (write_used[11:9] == 4'b0001) ? 1'b1 : 1'b0;
...
begin 
	 ...
	 if (RAND)
	  begin	// RAND set so de-ramdomize
		 if (INA[0])
			temp_ADC <= {~INA[13:1],INA[0],2'b0};
		 else
			temp_ADC <= {INA, 2'b0};
	  end
  else
    temp_ADC <= {INA, 2'b0};  // not set so just copy data	
end 
...
(.siga(Rx_frequency[1]), .rstb(C122_rst), .clkb(C122_clk), .sigb(C122_frequency_HZ[1]));	
	...
	.out_strobe(strobe[0]),
	//input
	.in_data(temp_ADC),					// Rx0 always connected to ADC output 
	//output
	.out_data_I(rx_I[0]),
	.out_data_Q(rx_Q[0])
	);
	...
	//control
	.clock(C122_clk),
	.sample_rate(Mux[1] ? C122_SampleRate[0]   : C122_SampleRate[1]), 
	.frequency(  Mux[1] ? C122_frequency_HZ[0] : C122_frequency_HZ[1]),   
	.out_strobe(strobe[1]),
	//input
	.in_data( Mux[1] ? temp_DACD : temp_ADC ),   // Rx1 connected to ADC or DAC when in PureSignal mode.
	//output
	.out_data_I(rx_I[1]),
	.out_data_Q(rx_Q[1])
	);
...
always @ (negedge _122MHz)
	DACD <= IO4 ? C122_cordic_i_out[21:8] : 14'd0;   // inhibit RF out when IO4 is low. 
...
// if break_in is slected then CW_PTT can generate RF otherwise PC_PTT must be active.			
assign FPGA_PTT = ((break_in & CW_PTT) | PC_PTT | debounce_PTT); // CW_PTT is used when internal CW is selected
...
// wire [15:0] user_analog	  = FPGA_PTT ? {4'b0,AIN4} : 16'b0;
 
CC_encoder #(50, NR) CC_encoder_inst (				// 50mS update rate, 10mS when TX_active
					//	inputs
...
//Led_flash Flash_LED10(.clock(CMCLK), .signal(icmp_rx_enable), .LED(DEBUG_LED10), .period(half_second));   //
