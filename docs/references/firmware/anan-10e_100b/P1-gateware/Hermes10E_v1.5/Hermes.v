...
// (C) Phil Harman VK6(A)PH, Kirk Weedman KD7IRS  2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014 
...
/* 	This program interfaces the LTC2208 to a PC over Ethernet.
	...
	Change log:
	 ...
	 2014 Nov 20   - Hermes V2.9 - Used as the basis for Hermes_14bit hardware.  
						- Extend 14 bits of ADC to 16 by setting low bits to zero.
						- Removed wideband data - uses too much space.
						- Try receiver2 so can run PureSignal - 93% full - OK
						- Added iFIR for Tx and enabled PureSignal selection - 94% full - OK
						- Set IP - works
						- Removed Program FPGA command, ASMI takes lots of space - use Bootloader instead - 93% full.
			 Dec 14  - Added clocks, CW and I2C Rx audio from new protocol to try and get more room.
			         - Enabled Wideband data
						- Reduced PHY_Rx_FIFO from 16k to 8k in order to increase Wideband FIFO to 8k 
						- Enabled DEBUG_LEDs
						- Reduced PHY_Rx_FIFO from 16k to 4k in order to increase Wideband FIFO to 16k - wont fit
						- PHY_Rx_FIFO =  Wideband FIFO = 8k
						- Add new protocol mic code 
	 2015  Jan  19 - Replaced ~PTT with clean_PTT_in i.e. debounce external PTT signal. 
						- Release as V1.1.
			 Feb   1 - Fixed Straight key mode.
						- Release as V1.2.	
			      11 - Fixed Line-in not being selected
						- Release as V1.3
	 2017  Apr   9 - Redo timing to remove glitch from TX RF output 
						- Added spi.v from Hermes to send Alex data twice at each data change
						- Release as V1.4
			 Apr	 9 - changed FW version number to v1.5
						- removed all max_delay and min_delay constraints from Hermes.sdc
						- compiled, retimed repetitively until timing closed
...
parameter  Hermes_serialno = 8'd15;		// Serial number of this version
...
//------------------------------------------------
//   SP_fifo  (8k words) dual clock FIFO
//------------------------------------------------

/*
        The spectrum data FIFO is 16 by 8k words long on the input.
...
//	When wide_spectrum is set and sp_fifo_wrempty then fill fifo with 8k words 
// of consecutive ADC samples.  Pass have_sp_data to Tx_MAC to indicate that 
// data is available.
// Reset fifo when !run or !wide_spectrum so the data always starts at a known state.


wire have_sp_data;

SP_fifo  SPF (.aclr(C122_rst | !run | !wide_spectrum), .wrclk (C122_clk), .rdclk(Tx_clock_2), 
...
audio_I2S audio_I2S_inst (.BCLK(CBCLK), .empty(Audio_empty), .LRCLK(CLRCLK), .data_in(Rx_audio), .data_out(CDIN), .get_data()); 	
...
// using 122.88MHz clock. If the count is less than the drive 
...
begin 
	...
	if (C122_Drive_Level >= PWM_count)
		DAC_ALC <= 1'b1;
	else 
		DAC_ALC <= 1'b0;
end 

// move Drive_Level into C122 clock domain
wire [7:0] C122_Drive_Level;
cdc_sync #(8) Drive_inst (.siga(IF_Drive_Level), .rstb(C122_rst), .clkb(C122_clk), .sigb(C122_Drive_Level)); 
...
begin 
  ...
  begin 										// Need to ensure that C&C data is stable 
	...
	begin
	  ...
	  PureSignal_enable <= IF_Rx_ctrl_2[6];		// decode what data to input to Rx2 during Tx; 0 = temp_ADC, 1 = DACD
...
Attenuator Attenuator_inst (.clk(CMCLK), .data(attenuator), .ATTN_CLK(ATTN_CLK), .ATTN_DATA(ATTN_DATA), .ATTN_LE(ATTN_LE));
...
// -------------------------- use the following for 2 receivers -------------------------------------------------		
//		// find max freq of the two receiver frequencies 
		if (C122_frequency_HZ[1] > C122_frequency_HZ[0] ) 
			 C122_freq_max <= C122_frequency_HZ[1];

		// find min freq of the five receiver frequencies
		if (C122_frequency_HZ[1] < C122_frequency_HZ[0] )
				C122_freq_min <= C122_frequency_HZ[1];
...
// should move Alex data into CBCLK domain, but since it changes slowly should be OK for now. 
wire Alex_SPI_SDO;
wire Alex_SPI_SCK;
wire SPI_RX_LOAD;
wire SPI_TX_LOAD;
...
// No Apollo board option so force Alex control 					  
assign SPI_SDO = Alex_SPI_SDO;		
assign SPI_SCK = Alex_SPI_SCK;		
assign J15_5   = SPI_RX_LOAD;			
assign J15_6   = SPI_TX_LOAD;     				  
