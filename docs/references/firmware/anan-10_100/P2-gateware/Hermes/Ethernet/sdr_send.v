  ...
  case (state)
		IDLE: 
			begin
				...
				if (discovery) begin
					response_tx_bits <= {32'd0, (8'd02 + {7'b0,run}), local_mac, board_type, protocol_version, code_version, 48'd0, number_Rx, 8'd1, 8'd0, beta_version}; 	// sequence number set to zero for future use
				...
				begin 				
					if (port_index == NR) begin	// send wideband data only when all  DDC data has been sent 
						...
						begin
							udp_tx_length <= 16'd1444; //PC_LENGTH + (samples_per_frame[k] * 16'd6) + 12;			
