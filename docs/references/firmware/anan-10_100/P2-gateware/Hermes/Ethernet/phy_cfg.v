...
always @(posedge clock)  
  begin
  if ((init_request || (allow_1Gbit != last_allow_1Gbit)) && !init_required)  begin
    init_required <= 1;
    values[8] <= {6'b0, allow_1Gbit, 9'b0};
    values[7] <= 16'h8104;
    values[6] <= 16'h5470;  // plus rx and tx clock delay, in 0.12 ns units to reg 104h
    values[5] <= 16'h8105;
    values[4] <= 16'h2222;			// Rx pad skews, reg 105h		
    values[3] <= 16'h8106;
    values[2] <= 16'h6666;			// Tx pad skews, reg 106h, added 25th Sept
    values[1] <= 16'h1300;
    values[0] <= 16'hxxxx;
    addresses[8] <= 9;
    addresses[7] <= 11;
