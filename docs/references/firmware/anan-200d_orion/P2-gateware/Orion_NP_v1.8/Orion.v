...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
		...
		Dec 29	- port of Angelia_15Dec2016 code to Orion using Quartus Prime Lite v16.0
	...
	Jan 1 2017	- enabled HW_timeout feature
		...
		Jan 18 	- hard coded inputs for the receiver modules as follows:
					...
					- added input switching option for DDC1 only (temp_DACD or temp_ADC[1]) to allow both 
		...
		Mar 9		- increased bootloader area in ASMI.v to 2MB vs 1MB, set erase boundary limit to 
					...
					- changed firmware version number to v1.0					
...
2018	Apr  4  - (N1GP) Merged changes from Orion_MkII protocol 2
					- changed FW version number to v1.2

2018	Dec  7  - (N1GP) Removed some conditional logic for setting Phy Rx/Tx timing skews
					- changed FW version number to v1.3

2018	Dec  23 - (N1GP) Swapped Rx Left & Right audio out to be aligned with P1
			 - Changed phy_cfg.v rx/tx clock skews from 40/77 to 30/70
			 - Disabled counter delay for leaving fifo_clear active in Mux_clear.v as it didn't seem to be needed
...
2018	Dec  25 - (N1GP) Noted xmit spurs on bands > 20 mtrs, didn't happen on v1.3
			 - Put receiver_inst1 back to using fifo_clear
			 ...
			 - Changed profile_CW so char_PTT stays active in state 0 if CW_char is found active
			   to avoid potential CW_PTT issues.
...
module Orion(
  ...
  input EXT_OSC_10MHZ,				// external 10MHz reference in
...
localparam board_type = 8'h04;		  	// 00 for Metis, 01 for Hermes, 02 for Griffin, 03 for Angelia, 04 for Orion, and 05 for Orion MkII
parameter  Orion_version = 8'd18;			// FPGA code version
...
//
//  Thus the logic in this always block is reversed from what you find in the Orion Mk II firmware, as the Mk II Orion schematic
...
CC_encoder #(50, NR) CC_encoder_inst (				// 50mS update rate
					...
					.User_ADC1 (user_analog),
					.User_ADC2 (16'b0),
					.User_IO ({3'b0, IO2, 1'b0, IO6, debounce_IO5, IO4}),
...
//C122_PLL PLL_inst (.inclk0(_122MHz), .c0(osc_80khz), .c1(_122_90), .locked());	
C122_PLL PLL_inst (.inclk0(_122MHz), .c0(osc_10MHZ), .locked());
	
//Apply to EXOR phase detector 
//assign FPGA_PLL = ref_80khz ^ osc_80khz; 
assign FPGA_PLL = osc_10MHZ ^ (ext_locked ? EXT_OSC_10MHZ : OSC_10MHZ);

assign OSC_ENABLE = ext_locked;	// enable local 10MHz osc module only if ext 10MHz is not present
...
Led_flash Flash_LED8(.clock(rx_clock), .signal(HB_counter[25]), .LED(DEBUG_LED8), .period(fast_clock_half_second));
...
Led_flash Flash_LED10(.clock(rx_clock), .signal(Audio_seq_err), .LED(DEBUG_LED10), .period(fast_clock_half_second));  // note fast clock now 
