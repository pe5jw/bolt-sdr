	 ...
	 4 Mar 2014		- Added bootloader mode option switching to pin 25 on J43 of the daughterboard header, OR'd with SW1
	  ...
	  13 Apr 2014	- Fixed bug with PureSignal implementation, timing
	   ...
	   4  May 2015 - Added external CW keying capability to iambic.v module via digital input IO4 
		...
		21 Jun 2015	- fixed bug with predistortion PureSignal functionality
	   ...
	   25 Jan 2016 - Moved external CW keying input to user digital input #2, IO5, pin 16 of J16, to remove pin assignment 
		...
		28 Jan 2016 - Added IO4 TX INHIBIT logic to DACD (Tx DAC) to prevent output when TX INHIBIT is active
...
module Orion(
  ...
  input  MODE2,                  //jumper J14 on Orion: 1 if removed = 1000T ethernet speed; 0 if jumpered = 100T speed; 
  input  ANT_TUNE,               //atu
...
parameter  Orion_version = 8'd52;		// Serial number of this version
...
wire C122_clk_3 = LTC2208_122MHz_3;
wire IF_clk;
wire CLRCLK;
...
//---------------------------------------------------------
/*
// transfer TLV input data to CMCLK domain & TLV output data to C122_clk domain
reg CMCLK_Mic_boost;
reg CMCLK_Line_In;
reg CMCLK_SSCK;
reg CMCLK_MOSI;
reg CMCLK_CMODE;
reg [4:0] CMCLK_Line_In_gain;

// transfer TLV inputs to CMCLK domain
cdc_sync #(1)
	cdc_Mic_boost (.siga(IF_Mic_boost), .rstb(C122_rst), .clkb(CMCLK), .sigb(CMCLK_Mic_boost)); // transfer Mic boost
cdc_sync #(1)
	cdc_Line_In (.siga(IF_Line_In), .rstb(C122_rst), .clkb(CMCLK), .sigb(CMCLK_Line_In)); // transfer Line In
cdc_sync #(5)
	cdc_Line_In_Gain (.siga(IF_Line_In_Gain), .rstb(C122_rst), .clkb(CMCLK), .sigb(CMCLK_Line_In_Gain)); // transfer Line In Gain

// transfer TLV outputs to C122_clk domain
cdc_sync #(1)
	cdc_CMODE (.siga(CMCLK_CMODE), .rstb(C122_rst), .clkb(C122_clk), .sigb(CMODE)); // transfer CMODE
cdc_sync #(1)
	cdc_SSCK (.siga(CMCLK_SSCK), .rstb(C122_rst), .clkb(C122_clk), .sigb(SSCK)); // transfer SSCK
cdc_sync #(1)
	cdc_MOSI (.siga(CMCLK_MOSI), .rstb(C122_rst), .clkb(C122_clk), .sigb(MOSI)); // transfer MOSI
cdc_sync #(1)
	cdc_nCS (.siga(CMCLK_nCS), .rstb(C122_rst), .clkb(C122_clk), .sigb(nCS)); // transfer nCS
*/
...
reg [15:0]temp_ADC[0:2];
reg [15:0] temp_DACD; // for pre-distortion Tx
...
begin 
   ...
   if (RAND) begin	// RAND set so de-ramdomize ...ADC1
		if (INA[0]) temp_ADC[0] <= {~INA[15:1],INA[0]};
		else temp_ADC[0] <= INA;
	end
   else temp_ADC[0] <= INA;  // not set so just copy data	 
		
	if (RAND_2) begin			// ...ADC2
		if (INA_2[0]) temp_ADC[1] <= {~INA_2[15:1], INA_2[0]};
		else temp_ADC[1] <= INA_2;	
	end
	else temp_ADC[1] <= INA_2;

	if (RAND_3) begin			// ...ADC3
		if (INA_3[0]) temp_ADC[2] <= {~INA_3[15:1], INA_3[0]};
		else temp_ADC[2] <= INA_3;	
	end
	else temp_ADC[2] <= INA_3;
...
assign Rx3_phase_word = common_Merc_freq ? C122_sync_phase_word[0] : C122_sync_phase_word[2];
...
assign select_input_RX[0] = (ADC_RX1 == 2'b01) ? temp_ADC[1] : ((ADC_RX1 == 2'b10) ? temp_ADC[2] : temp_ADC[0]);
assign select_input_RX[1] = (ADC_RX2 == 2'b01) ? temp_ADC[1] : ((ADC_RX2 == 2'b10) ? temp_ADC[2] : temp_ADC[0]);
assign select_input_RX[2] = (ADC_RX3 == 2'b01) ? temp_ADC[1] : ((ADC_RX3 == 2'b10) ? temp_ADC[2] : temp_ADC[0]);
...
receiver receiver_inst6(   // Rx7
	...
	cdc_mcp #(48)			// Transfer the receiver data and strobe from C122_clk to IF_clk
		IQ_sync1 (.a_data ({rx_I[0], rx_Q[0]}), .a_clk(C122_clk),.b_clk(IF_clk), .a_data_rdy(strobe[0]),
				.a_rst(C122_rst), .b_rst(IF_rst), .b_data(IF_M_IQ_Data[0]), .b_data_ack(IF_M_IQ_Data_rdy[0]));
	
	cdc_mcp #(48)			// Transfer the receiver data and strobe from C122_clk to IF_clk
		IQ_sync2 (.a_data ({rx_I[1], rx_Q[1]}), .a_clk(C122_clk),.b_clk(IF_clk), .a_data_rdy(strobe[1]),
				.a_rst(C122_rst), .b_rst(IF_rst), .b_data(IF_M_IQ_Data[1]), .b_data_ack(IF_M_IQ_Data_rdy[1]));

	cdc_mcp #(48)			// Transfer the receiver data and strobe from C122_clk to IF_clk
		IQ_sync3 (.a_data ({rx_I[2], rx_Q[2]}), .a_clk(C122_clk),.b_clk(IF_clk), .a_data_rdy(strobe[2]),
				.a_rst(C122_rst), .b_rst(IF_rst), .b_data(IF_M_IQ_Data[2]), .b_data_ack(IF_M_IQ_Data_rdy[2]));

				cdc_mcp #(48)			// Transfer the receiver data and strobe from C122_clk to IF_clk
		IQ_sync4 (.a_data ({rx_I[3], rx_Q[3]}), .a_clk(C122_clk),.b_clk(IF_clk), .a_data_rdy(strobe[3]),
				.a_rst(C122_rst), .b_rst(IF_rst), .b_data(IF_M_IQ_Data[3]), .b_data_ack(IF_M_IQ_Data_rdy[3]));

	cdc_mcp #(48)			// Transfer the receiver data and strobe from C122_clk to IF_clk
		IQ_sync5 (.a_data ({rx_I[4], rx_Q[4]}), .a_clk(C122_clk),.b_clk(IF_clk), .a_data_rdy(strobe[4]),
				.a_rst(C122_rst), .b_rst(IF_rst), .b_data(IF_M_IQ_Data[4]), .b_data_ack(IF_M_IQ_Data_rdy[4]));

	cdc_mcp #(48)			// Transfer the receiver data and strobe from C122_clk to IF_clk
		IQ_sync6 (.a_data ({rx_I[5], rx_Q[5]}), .a_clk(C122_clk),.b_clk(IF_clk), .a_data_rdy(strobe[5]),
				.a_rst(C122_rst), .b_rst(IF_rst), .b_data(IF_M_IQ_Data[5]), .b_data_ack(IF_M_IQ_Data_rdy[5]));

	cdc_mcp #(48)			// Transfer the receiver data and strobe from C122_clk to IF_clk
		IQ_sync7 (.a_data ({rx_I[6], rx_Q[6]}), .a_clk(C122_clk),.b_clk(IF_clk), .a_data_rdy(strobe[6]),
				.a_rst(C122_rst), .b_rst(IF_rst), .b_data(IF_M_IQ_Data[6]), .b_data_ack(IF_M_IQ_Data_rdy[6]));
...
//    ADC SPI interface 
//---------------------------------------------------------
// generate a 30.72MHz clock for the Orion_ADC module, results in 7.68MHz clock for the ADC78H90 chip
wire userADC_clk;
PLL_30MHz PLL_30MHz_inst(.inclk0(_122MHz), .c0(userADC_clk));
...
Orion_ADC ADC_SPI(.clock(userADC_clk/*CBCLK*/), .SCLK(ADCCLK), .nCS(nADCCS), .MISO(ADCMISO), .MOSI(ADCMOSI),
...
always @ (negedge C122_clk)
	DACD <= IO4 ? C122_cordic_i_out[13:0] : 14'b0;   //gain of 4 // shut down TX DAC if TX INHIBIT is active (IO4)
...
Orion_Tx_fifo_ctrl #(RX_FIFO_SZ, TX_FIFO_SZ) TXFC 
           (IF_rst, IF_clk, IF_tx_fifo_wdata, IF_tx_fifo_wreq, IF_tx_fifo_full,
            ...
            AIN3, AIN4, AIN6, IO4, IO5, IO6, IO8, VNA_start, VNA, pk_detect_reset, pk_detect_ack);
...
reg	[4:0] Orion_atten3;		// attenuation setting for input attenuator 3 (input atten for ADC3), 0-31 dB
reg			atten3_enable; 		//enable/disable control for input attenuator 3 (0=disabled, 1= enabled)
...
begin 
  ...
  begin // set up default values - 0 for now
    ...
    {IF_DFS1, IF_DFS0} <= 2'b00;   	// decode speed 
	 ...
	 Orion_micPTT_disable <= 1'b1;   // Orion mic PTT disable; initialize hardware w/disable activated
    // RX_CONTROL_2
    ...
    IF_Drive_Level     <= 8'b0;	   // drive at minimum
	 ...
	 Orion_atten3			<= 5'b0;		// default attenuation setting for input attenuator 3 (input atten for ADC3)
	 atten3_enable 		<= 1'b0;		// default disable input attenuator 3 
    ...
    keyer_spacing      <= 1'b0;	   // 0 = off, 1 = on
	 ...
	 PS_enabled			  <= 1'b0;		// default PS disabled
  end
  ...
  begin 										// Need to ensure that C&C data is stable 
	...
	begin
	  ...
	  Orion_atten3		<= IF_Rx_ctrl_2[4:0];	// attenuation setting for input attenuator 3 (input atten for ADC3)
	  atten3_enable		<= IF_Rx_ctrl_2[5];		// input attenuator 3 enable/disable (0 = disabled, 1 = enabled)
...
// IO4 is a hardware TX INHIBIT feature, if IO4 low TX output is inhibited; default IO4 is high due to pull-up resistors
// in this firmware, IO4 also switches the radio into TX mode to provide protection for the radio when nearby connected 
// radios transmit with high power in a multi-TX station
assign FPGA_PTT = IO4 && (IF_Rx_ctrl_0[0] || CW_PTT || clean_PTT_in); // IF_Rx_ctrl_0 only updated when we get correct sync sequence. CW_PTT is used when internal CW is selected

//------------------------------------------------------------
//  Orion on-board attenuators 
//------------------------------------------------------------

// set the three input attenuators
wire [4:0] atten_data_in;
wire [4:0] atten2_data_in;
wire [4:0] atten3_data_in;
wire [4:0] attenuator1;
wire [4:0] attenuator2;
wire [4:0] attenuator3;

assign atten_data_in = Orion_atten_enable ? Orion_atten : (Preamp ? 5'b0_0000 : 5'b1_0100);
assign atten2_data_in = atten2_enable ? Orion_atten2 : 5'b0_0000;
assign atten3_data_in = atten3_enable ? Orion_atten3 : 5'b0_0000;
assign attenuator1 = FPGA_PTT ? atten_on_Tx : atten_data_in;
assign attenuator2 = FPGA_PTT ? atten_on_Tx : atten2_data_in;
assign attenuator3 = FPGA_PTT ? atten_on_Tx : atten3_data_in;
...
Attenuator Attenuator_ADC3 (.clk(CMCLK), .data(attenuator3), .ATTN_CLK(ATTN_CLK_3), .ATTN_DATA(ATTN_DATA_3), .ATTN_LE(ATTN_LE_3));
...
always @ (posedge C122_clk) begin
	if (C122_cbrise) begin
		...
		// find max freq of the seven receiver frequencies ... five temporarily
...
//
wire				 clean_IO5;						// debounced IO5 CW input
...
EXT_C10_PLL PLL3_inst (.inclk0(EXT_OSC_10MHZ), .c0(), .locked(ext_locked));
...
assign FPGA_PLL = (ext_locked ? EXT_OSC_10MHZ : OSC_10MHZ) ^ osc_10MHz;
