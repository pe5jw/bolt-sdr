...
module General_CC 
			( 	
			   ...
			   output reg [15:0] Rx_Specific_port,
				...
				output reg [1:0]  _10MHz_reference,
				output reg common_merc_freq,
				output reg _128MHz_reference,
		...
		PROCESS:
			begin
				case (byte_number) 	//save balance of sequence number
					  ...
					  57: _10MHz_reference <=  udp_rx_data[1:0];
					  58: begin
								PA_enable         <= udp_rx_data[0];
								Apollo_enable     <= udp_rx_data[1]; 
								common_merc_freq  <= udp_rx_data[2]; 
								_128MHz_reference <= udp_rx_data[3]; 
								data_ready        <= 1'b1;
							end 
					  59: begin	
