...
parameter  Metis_version = 8'd40;	// FPGA code version
...
reg  [1:0] RxN_preamps;		// Mercury boards preamp states (0000=all preamps OFF, 0001=Merc1 preamp ON, 0010=Merc2 preamp ON, etc)
reg	Merc_atten_on_Tx; // 0 = disabled, 1 = enabled

assign RxN_preamps = (FPGA_PTT & Merc_atten_on_Tx) ? 2'b0 : ~Mercury_Attenuator[1:0];
...
/*
        ...
        <[53:47]OC><[46:15]Alex_data><[14]DITHER><[13]RAND><[12:5]Drive_Level><[4]Alex_enable><[3:2]RxN_preamps><[1:0]adc2>
*/
assign IF_xmit_data = {IF_SyncRx[0][1],merc1_2_pres,sampling_rate[CC_address],FPGA_PTT,CC_address,IF_phase_word[CC_address],_128MHz_reference,_10MHz_reference,
                       Open_Collector[7:1],IF_Alex_data,IF_DITHER, IF_RAND,Drive_Level,Alex_enable,RxN_preamps,IF_RxADC1,IF_RxADC0};

NWire_xmit  #(.OSL(97), .OSW(1), .ICLK_FREQ(48000000), .XCLK_FREQ(48000000), .SEND_FREQ(10000)) 
...
cdc_mcp #(32) Alex_cdc_inst
  (.a_rst(C125_rst), .a_clk(rx_clock), .a_data(C125_Alex_data), .a_data_rdy(Rx_data_ready), .b_rst(IF_rst), .b_clk(IF_clk), .b_data(IF_Alex_data));
...
reg [26:0]HB_counter;
always @(posedge PHY_CLK125) HB_counter = HB_counter + 1'b1;
assign RAM_A3 = HB_counter[25];  // Blink
assign RAM_A4 = ~(IF_SyncRx[0][1]);
assign RAM_A5 = ~(SyncRx[0][1]);
