...
module High_Priority_CC 
			( 	
				...
				output reg Dot,
				output reg Dash,
				output reg [31:0]Rx_phase_word[0:NR-1] /* ramstyle = "logic" */,		
				output reg [31:0]Tx0_phase_word,
...
reg [31:0]temp_Rx_phase_word[0:NR-1];
	...
	begin
		if (udp_rx_active && to_port == port)	// look for to_port = 1027
			case (state)
				PROCESS:
				case (byte_number)
						i*4 + 9:	temp_Rx_phase_word [i][31:24] <= udp_rx_data; 
						i*4 + 10:	temp_Rx_phase_word [i][23:16] <= udp_rx_data;
						i*4 + 11:	temp_Rx_phase_word [i][15:8]  <= udp_rx_data;
						i*4 + 12:	temp_Rx_phase_word [i][7:0]   <= udp_rx_data;
						350:		Rx_phase_word[i] <= temp_Rx_phase_word[i];
			...
			begin
				case (byte_number) 	//save balance of sequence number
						...
						5: begin
						 ...
						 329:	Tx0_phase_word [31:24]  <= udp_rx_data;
						 330:	Tx0_phase_word [23:16]  <= udp_rx_data;					
						 331:	Tx0_phase_word [15:8]   <= udp_rx_data;
						 332:	Tx0_phase_word [7:0]    <= udp_rx_data;	
