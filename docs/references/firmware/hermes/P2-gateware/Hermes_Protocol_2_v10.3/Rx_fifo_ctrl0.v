...
module Rx_fifo_ctrl0(
	...
	input spd_rdy,							// DDC for RF has data
	input spd_rdy2,						// DDC for DAC has data
	input fifo_full,
...
if (reset) begin 
	...
	// If PureSignal selected then Sync != 0. In which case both RF and DAC feedback DDC MUST have data ready.
	
	1:	begin
			if (fifo_full) state <= 14; 		// clear fifo, will need to do this if code has been idle
			else if((Sync == 0 && spd_rdy) || (Sync != 0 && spd_rdy && spd_rdy2)) begin 								
					wrenable <= 1'b1;
  ...
  14: begin
		fifo_clear <= 1'b1;
		if(Rx_fifo_empty) state <= 0;   // ensure Rx fifo is empty 
		end
