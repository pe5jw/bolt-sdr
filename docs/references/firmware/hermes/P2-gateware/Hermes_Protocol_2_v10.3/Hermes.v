...
wire speed = 1'b0; // high for 1000T, low for 100T
// enable AF Amp
...
parameter  Hermes_version = 8'd103;	// FPGA code version
...
//assign IF_rst = (network_state < 4'h8);  // hold code in reset until Ethernet code is running.
assign IF_rst = network_state;
...
reg [27:0] sec_count;
wire HW_timeout;
always @ (posedge rx_clock)  // 25MHz at 100T 
begin
	if (HW_timer_enable) begin 
		if (timer_reset) sec_count <= 0;
		else if (sec_count != 28'd25_000_000) 	// approx 2 secs. 
					sec_count <= sec_count + 28'd1;
	end 
	else sec_count <= 28'd0;
end
assign HW_timeout = (sec_count >= 28'd25_000_000) ? 1'd1 : 1'd0;
...
sdr_send #(board_type, NR, protocol_version) sdr_send_inst(
	//inputs
	.tx_clock(tx_clock),
	.udp_tx_active(udp_tx_active),
...
/*
     ...
     fifo_rdreq[n]	|rdreq		  q[7:0]| Rx_data[n]
	     ...
	     tx_clock		|>rdclk		rdempty | Rx_fifo_empty[n]
		               |                   |
						   |		 rdusedw[11:0]| Rx(n)_used*  (0 to 2048 bytes)
						   +-------------------+
						   |                   |
...
// This is just for Rx0 since it can Muxed with Rx1.
		...
		Rx_fifo Rx0_fifo_inst(.wrclk (C122_clk),.rdreq (fifo_rdreq[0]),.rdclk (tx_clock),.wrreq (Rx_fifo_wreq[0]), 
							 .data (Rx_fifo_data[0]), .q (Rx_data[0]), .wrfull(Rx_fifo_full[0]), .rdempty(Rx_fifo_empty),
							 .rdusedw(Rx_used[0]), .aclr (IF_rst | Rx_fifo_clr[0] | !run | fifo_clear));  // *** added fifo_clear											
							 
		Rx_fifo_ctrl0 #(NR) Rx0_fifo_ctrl_inst(.clock2(rx_clock), .reset(!run || !EnableRx0_7[0] | fifo_clear ), .clock(C122_clk), .data_in_I(rx_I[1]), .data_in_Q(rx_Q[1]),
													.spd_rdy(strobe[0]), .spd_rdy2(strobe[1]), .fifo_full(Rx_fifo_full[0]), .Rx_fifo_empty(Rx_fifo_empty),
													.wrenable(Rx_fifo_wreq[0]), .data_out(Rx_fifo_data[0]), .fifo_clear(Rx_fifo_clr[0]),
													.Sync_data_in_I(rx_I[0]), .Sync_data_in_Q(rx_Q[0]), .Sync(Mux));
													
		assign  fifo_ready[0] = (Rx_used[0] > 12'd1427) ? 1'b1 : 1'b0;  // used to signal that fifo has enough data to send to PC

// This is for Rx1. 

		Rx_fifo Rx_fifo_inst(.wrclk (C122_clk),.rdreq (fifo_rdreq[1]),.rdclk (tx_clock),.wrreq (Rx_fifo_wreq[1]), 
							 .data (Rx_fifo_data[1]), .q (Rx_data[1]), .wrfull(Rx_fifo_full[1]),
							 .rdusedw(Rx_used[1]), .aclr (IF_rst | Rx_fifo_clr[1] | !run | fifo_clear));   
		...
		Rx_fifo_ctrl #(NR) Rx0_fifo_ctrl_inst2( .reset(!run || !EnableRx0_7[1] ), .clock(C122_clk),
		...
		assign  fifo_ready[1] = (Rx_used[1] > 11'd1427) ? 1'b1 : 1'b0;  // used to signal that fifo has enough data to send to PC


// Whenever Mux changes state clear Rx FIFOs. 
Mux_clear Mux_clear_inst (.clock(rx_clock), .Mux(Mux), .Rx_fifo_empty(Rx_fifo_empty), .fifo_clear(fifo_clear));
...
wire [17:0] update_rate = 12500;   // force 100T update rate. 
...
/*
			 ...
			 Tx1_IQ_data   |data[47:0]	         | 
								|				         |
			Tx1_fifo_wrreq |wrreq  wrusedw[12:0]|	write_used[12:0]*	
								|					      |									    
				 rx_clock	|>wrclk	 		      |
								+--------------------+								
...
wire almost_full 	= (write_used[12:8] == 5'b01111) ? 1'b1 : 1'b0;  // >= 3,840 samples
wire almost_empty = (write_used[12:9] == 5'b00001) ? 1'b1 : 1'b0;  // <= 511 samples
...
/*
 ...
 EPCS_FIFO_enable  |wrreq		         | 
					    |					      |									    
	    rx_clock	 |>wrclk wrusedw[10:0]| EPCS_wrused*
					    +-------------------+								
	   EPCS_rdreq   |rdreq		  q[7:0] | EPCS_data
					    |					      |					  			
			     	    |   		            |  
			          |                   | 							
     clock_12_5MHz |>rdclk rdusedw[10:0]| EPCS_Rx_used*	    
					    +-------------------+								
					    |                   |
			  IF_rst  |aclr               |								
					    +-------------------+						
...
// ***** test 
//assign select_input_RX[1] = Mux == 8'd0 ? temp_ADC : temp_DACD; 
...
//(.siga(Rx_frequency[2]), .rstb(C122_rst), .clkb(C122_clk), .sigb(C122_frequency_HZ[2]));

//cdc_sync #(32) Rx_freq3 
//(.siga(Rx_frequency[3]), .rstb(C122_rst), .clkb(C122_clk), .sigb(C122_frequency_HZ[3]));	
...
//	receiver3 receiver_inst2(   
//	//control
//	.clock(C122_clk),
//	.sample_rate(C122_SampleRate[2]),
//	.frequency(C122_frequency_HZ[2]),     
//	.out_strobe(strobe[2]),
//	//input
...
//	receiver3 receiver_inst3(   
//	//control
//	.clock(C122_clk),
//	.sample_rate(C122_SampleRate[3]),
//	.frequency(C122_frequency_HZ[3]),     
//	.out_strobe(strobe[3]),
//	//input
...
cpl_cordic # (.IN_WIDTH(17))
 		cordic_inst (.clock(_122MHz), .frequency(C122_frequency_HZ_Tx), .in_data_I(I), // fixed 10MHz phase word - was C122_frequency_HZ_Tx, 
		.in_data_Q(Q), .out_data_I(C122_cordic_i_out), .out_data_Q());						 // phase_word[31:0] =  2^32 * frequency(Hz)/DSP clock frequency (Hz) = 349,525,333
...
// using PHY 125MHz clock. If the count is less than the drive 
...
wire osc_80khz;
wire locked_10MHz;
wire DAC_clock_15;       // 15 Degree shifted clock for DAC.
...
C122_PLL PLL_inst (.inclk0(_122MHz), .c0(osc_80khz), .c1(DAC_clock_15), .locked());	
...
//Led_flash Flash_LED10(.clock(CMCLK), .signal(fifo_clear), .LED(DEBUG_LED10), .period(half_second));   //
...
// assign DEBUG_LED9 = !Mux[1];
assign DEBUG_LED9  = !HW_timer_enable;
assign DEBUG_LED10 = !timer_reset;
