...
## AMENDMENT — issue #1302: receiver-count transitions (OVERRIDES §1 and §2(d))

**Field failure history.** §1/§2(d)'s original "4 DDC only while PS+MOX" shape flipped the EP6 framing at every TR edge and field-failed (#1283/#1285). The `ff9ce060` correction made the framing **arm-scoped** (4-DDC for the whole armed period) — but still flipped the count 1↔4 **on the live stream** at the arm/disarm click, and that is what froze the tester's radio in **#1302**.

**The gateware mechanism (why any live flip is fatal — it is frame-length, not C&C offsets).** The C10 gateware latches the count instantly mid-frame (`IF_last_chan <= IF_Rx_ctrl_4[5:3]`, Hermes.v:2151) into a free-running EP6 frame builder whose loop count switches combinationally 62↔18 (Hermes_Tx_fifo_ctrl.v:140-152) with an inequality terminator (`loop_cnt != num_loops`). One wrong-length frame permanently shifts the byte stream relative to the frame-blind 1024-byte Tx_MAC packetizer (Tx_MAC.v:998, 1026-1031). The EP6 Tx FIFO is never cleared while `run=1` (clear only via AD_ERR overflow, `aclr = IF_rst || IF_tx_fifo_clr`, Hermes.v:1814), so the corruption is **permanent**: sync never re-lands at payload offsets 8/520 and Zeus discards 100% of EP6 while `p1.tx.rate` stays perfect. `IF_last_chan` also persists across stop/start (reset only by `IF_rst` = PLL/power), so reconnects compound the damage.

**The shipped model (this branch): stop/drain/restart — never a live flip.**
- `Protocol1Client.SetPsEnabledAsync` is the only way to change PS arm state on a live C10 stream. It is idempotent (mode already matches ⇒ no wire traffic) and single-flight.
- Transition = pause EP2 → **stop ×3 on all platforms** → drain ≥100 ms (inbound discarded) → atomically reconfigure (numRx byte + parser format + full RX parser-state reset) → **pre-announce** the new count in EP2 Config frames while `run=0` → start (+ retry up to 3× if no valid EP6 within ~1 s) → resume EP2. Modeled on piHPSDR `tx_ps_onoff` → `old_protocol_stop(); usleep(100000); flip; old_protocol_run()` (transmitter.c:2505-2511; "OZY's tend to hang if the number of receivers is changed while running", old_protocol.c:1053-1062).
- Pre-announce works because EP2 C&C decode is **not** gated on `run`: Rx_MAC.v routes EP2 to SEND_TO_FIFO unconditionally and the Hermes.v SYNC state machine is gated only on `IF_rst`/`IF_PHY_drdy` (verified in this pass; same trick as Thetis `ForceCandCFrames`, networkproto1.c:106-130).
- Why stop realigns a corrupted stream: with `run=0` the MAC stops draining, the builder overflows the FIFO within ms, AD_ERR fires `Tx_fifo_clr` at a frame boundary — restart resumes frame-aligned.
- **Connect-while-armed** starts directly in 4-DDC: RadioService seeds `SetPsEnabled(state.PsEnabled)` *before* `StartAsync`, `StartAsync` pre-announces the count while `run=0` (correcting a stale `IF_last_chan` from a prior session), and the post-connect resync is a wire no-op via idempotence. No code path performs a live flip on C10.
- **Connect hygiene (stale-run guard):** every C10 `StartAsync` first sends stop ×3 + drains before the pre-announce. If a previous host crashed while armed, the radio can still be streaming (`run=1`, its ICMP never arrived) — with `run=1` the Tx FIFO never overflow-clears, so pre-announcing into it would be a live flip from packet one. The stop is idempotent (`run <= 0`) on an already-stopped radio.
- **Teardown is serialized with transitions:** `StartAsync`/`StopAsync`/`SetPsEnabledAsync` share one gate; `StopAsync` cancels the loop token first (aborting an in-flight transition's drain — it re-checks liveness and never sends start into a stopping session) and cancels the F3 start-handshake watchdog, so no stray start can ever follow the teardown stop and orphan a stream at an abandoned port.
- **Watchdog:** while armed on C10, 2 s of zero parseable 4-DDC packets with datagrams still arriving ⇒ `PsFeedbackStalled` ⇒ RadioService auto-disarms through the normal StateDto flow (the disarm transition itself realigns the radio).
- **Observability:** 4-DDC sync-parse failures are counted and logged at 1 Hz (`p1.rx.ps4ddc pkts=… ok=… fail=… dropped=…`), with a rate-limited WARN on sustained failure — the silent bare-return discard that blinded the #1302 log is gone.

**HL2 is different — deliberately untouched.** HL2's EP6 builder is frame-aware and sync-safe by construction (fixed 512-byte countdown, usopenhpsdr1.v:395-480; `>=` tlast comparison in radio.v), so its shipped MOX-scoped 4-DDC flip cannot shift sync and stays exactly as-is. All HL2 wire behavior remains byte-identical.
