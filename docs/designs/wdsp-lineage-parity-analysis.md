...
wrapper schema v5 mirrors the same primary-action fields as `triagePrimaryAcceptance*`, and strict
validation cross-checks them against `validation-triage-report.json`, so a blocked wrapper summary
can be handed directly to the operator or UI without losing the next command. The wrapper console
output and the `Live Acceptance Cycle Summary` Markdown section print the primary action, command or
manual action, and follow-up so the G2 operator does not have to inspect nested JSON before the next
capture.
...
from `acceptanceEvidenceReady`: offline review only requires the comparison, packaged runtime audit,
and runtime hashes to line up, while acceptance evidence also requires `-ValidateBundle`,
