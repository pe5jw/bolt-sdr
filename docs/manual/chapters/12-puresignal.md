...
PureSignal is available on Protocol-2 boards (ANAN-G2, G2 MkII, Orion-class) and on the Hermes-Lite 2 over Protocol-1. A few details differ by board, and those are called out below. Two single-ADC boards — the **ANAN-10E** and **ANAN-G2E** — now have **experimental** PureSignal that works from the normal PS button just like every other board; see the note at the end of this chapter.
...
The **ANAN-10E (HermesII)** and **ANAN-G2E (HermesC10)** have only **one** receive ADC. PureSignal normally needs a second receiver to listen to the amplifier's output, which is why these boards never offered it before. Zeus now *time-multiplexes* the single ADC — briefly borrowing it for the feedback sample during transmit — so these radios can run predistortion. **You use it exactly like any other board: arm PureSignal with the PS button and it engages.** There is no special switch.

Two things happen automatically to keep this safe on a single-ADC radio:

- The ADC is shared between your receiver and the feedback, so the feedback is taken **only during a transmit burst** and the ADC reverts to your normal receive the instant you unkey.
- On key-down with PureSignal armed, Zeus seeds a **protective attenuation** on that ADC's transmit-time input so the amplifier's coupler can't overload it.

> **This is experimental — please report what you see.** The single-ADC path is validated end-to-end against Zeus's virtual-radio test harness (a software radio that speaks the real HPSDR wire protocol), but it has had limited time on real 10E / G2E hardware. As always, PureSignal starts **off** every session and you arm it by hand. Bring your drive up gradually the first time, watch the convergence dial, and if you own one of these radios please tell us how it behaves on the air via the [Zeus Discord or GitHub](https://github.com/OpenHPSDR-Zeus-org/openhpsdr-zeus). The dual-ADC G2 / Orion-class PureSignal is unchanged.
