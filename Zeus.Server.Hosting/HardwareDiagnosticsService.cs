...
public sealed class HardwareDiagnosticsService : IHostedService, IDisposable
{
    ...
    [
        ...
        new
        {
            ...
            status = "candidate",
            notes = "Thetis gates Alex/Apollo/MKII/Orion options by model. Zeus should surface only controls backed by BoardCapabilities and persisted per radio; G2 ADC1/RX2 attenuation is now exposed, while ADC2 ground-on-TX and 10-DDC assignment stay read-only until their protocol/UI mapping is verified. G2 Dig In TX Disable is decoded as read-only diagnostics first; TX blocking must remain opt-in and board-gated.",
        },
        new
        {
