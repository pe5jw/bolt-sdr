...
- **`docs/references/firmware/` — READ THIS FOR ANY DRIVE / PA / TX / protocol / wire-byte / per-board change.** It is a source-only mirror of the FPGA gateware + MCU firmware for **every** P1/P2 radio Zeus supports (all ~29 boards, no gaps). Start at [`docs/references/firmware/README.md`](docs/references/firmware/README.md) — the coverage map resolves each radio to its source directory — then grep `docs/references/firmware/<board>/` for what the board *actually does on the wire/gateware*. Because Zeus is bench-tested only on the G2, this is how non-G2 boards stay correct **by construction** instead of by hope: confirm the real RTL/C before changing anything board-agnostic in the drive/PA/TX path. Reading it on every coding session is the single highest-leverage way to avoid shipping a fix that's right for the G2 and silently wrong on Hermes/HL2/Orion-MkII/etc. Shared gateware builds share one dir (e.g. all G2 variants → `anan-g2_saturn/`, the Orion-MkII family → `orion-mkii_7000-8000dle/`).
