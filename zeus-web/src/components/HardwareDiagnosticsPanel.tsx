...
export function HardwareDiagnosticsPanel() {
  ...
  const loadEndpointDiagnostics = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const [nextKeying, nextSupply, nextThermal, nextG2Sensors, nextNetwork, nextTx, nextSmartNr, nextDspLive, nextLabels, nextActions, nextDigIn] =
      ...
      const failures = [nextKeying, nextSupply, nextThermal, nextG2Sensors, nextNetwork, nextTx, nextSmartNr, nextDspLive, nextLabels, nextActions, nextDigIn]
        .filter(
  ...
  const dspFields: Field[] = [
    ...
    { label: 'TX Preview', value: boolLabel(dsp?.txMonitorRequested) },
  ...
  const audioFields: Field[] = [
    ...
    { label: 'TX Preview', value: boolLabel(audio?.txMonitorRequested) },
    { label: 'Squelch', value: boolLabel(audio?.squelchEnabled) },
    { label: 'SQL Mode', value: audio?.squelchMode },
    { label: 'SQL Gate Source', value: audio?.squelchGateSource },
    { label: 'SQL Open Known', value: boolLabel(audio?.squelchOpenKnown) },
