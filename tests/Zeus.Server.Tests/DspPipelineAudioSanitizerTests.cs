...
public sealed class DspPipelineAudioSanitizerTests
{
    ...
    [Fact]
    public void ApplyRxAudioLeveler_LiftsPassbandBackedNr5WarmupBlackout()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = -6.3
        ...
        float[] block = new float[1024];
        Array.Fill(block, DbToLinear(-26.4));
        block[0] = DbToLinear(-22.2);
        var nr5 = Nr5Diagnostics(
            signalConfidence: 0.159,
            signalProbability: 0.021,
            agcGate: 0.0,
            recoveryDrive: 0.0,
            weakSignalMemory: 0.0,
            outputDbfs: -76.4,
        ...
        Assert.True(
            state.DesiredGainDb >= 7.0,
            $"desired={state.DesiredGainDb:F2}, applied={state.AppliedGainDb:F2}, output={state.OutputRmsDbfs:F1}, gainDelta={state.GainDeltaDb:F2}");
        Assert.True(
            state.AppliedGainDb >= 7.0,
            $"desired={state.DesiredGainDb:F2}, applied={state.AppliedGainDb:F2}, output={state.OutputRmsDbfs:F1}, gainDelta={state.GainDeltaDb:F2}");
        Assert.InRange(state.OutputRmsDbfs, -20.5, -17.0);
        Assert.False(state.OutputLimited);
    }
    ...
    public void ApplyRxAudioLeveler_DoesNotLiftNr5WarmupBlackoutWithoutPassbandProof()
    {
        ...
        Assert.True(
            ...
            $"desired={state.DesiredGainDb:F2}, applied={state.AppliedGainDb:F2}, output={state.OutputRmsDbfs:F1}, gainDelta={state.GainDeltaDb:F2}");
        Assert.True(state.OutputRmsDbfs <= -25.0, $"output={state.OutputRmsDbfs:F1}");
        Assert.False(state.OutputLimited);
    }
    ...
    [Fact]
    public void ApplyRxAudioLeveler_NormalizesNearFieldStrongNr5SpeechFromLive40mTrace()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 20.7,
        ...
        Assert.InRange(state.DesiredGainDb, 18.0, 20.5);
        Assert.Equal(state.DesiredGainDb, state.AppliedGainDb, precision: 6);
        Assert.InRange(state.OutputRmsDbfs, -21.0, -18.0);
        Assert.False(state.OutputLimited);
    }
    ...
    [Fact]
    public void ApplyRxAudioLeveler_NormalizesNearFieldStrongNr5SpeechWhenAgcGateLags()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 19.3,
        ...
        Assert.InRange(state.DesiredGainDb, 16.5, 18.8);
        Assert.Equal(state.DesiredGainDb, state.AppliedGainDb, precision: 6);
        Assert.InRange(state.OutputRmsDbfs, -21.0, -18.5);
        Assert.False(state.OutputLimited);
    }
    ...
    [Fact]
    public void ApplyRxAudioLeveler_NormalizesPeakProvenG2SpeechAboveHotTailCap()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 9.8,
        ...
        Assert.InRange(state.DesiredGainDb, 20.0, 22.8);
        Assert.Equal(state.DesiredGainDb, state.AppliedGainDb, precision: 6);
        Assert.InRange(state.OutputRmsDbfs, -20.5, -17.0);
        Assert.False(state.OutputLimited);
    }
    ...
    [Fact]
    public void ApplyRxAudioLeveler_NormalizesStrongPassband7305SpeechToTarget()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 17.4,
        ...
        Assert.InRange(state.DesiredGainDb, 14.5, 17.0);
        Assert.Equal(state.DesiredGainDb, state.AppliedGainDb, precision: 6);
        Assert.InRange(state.OutputRmsDbfs, -21.0, -18.5);
        Assert.False(state.OutputLimited);
    }
    ...
    [Fact]
    public void ApplyRxAudioLeveler_NormalizesHeldStrongNativeNr5SpeechValleyFromLive40mTrace()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 6.6,
        ...
        float[] block = new float[1024];
        Array.Fill(block, DbToLinear(-38.9));
        var nr5 = Nr5Diagnostics(
            signalConfidence: 0.432,
            signalProbability: 0.734,
            agcGate: 0.931,
            recoveryDrive: 1.000,
            weakSignalMemory: 0.668,
            outputDbfs: -26.2,
            inputDbfs: -55.9,
            maskSmoothing: 0.353,
            peakEvidence: 0.987,
            outputPeakDbfs: -14.2,
            adjacentNoiseTrust: 0.515,
            adjacentNoiseDrive: 0.463,
            levelDrive: 0.997);
        ...
        DspPipelineService.ApplyRxAudioLeveler(
            ...
            ref state,
            nr5,
            FrontendTopPeak(offsetHz: 169_875, snrDb: 43.5, dbfs: -70.9, confidence: 0.940),
            filterLowHz: -3_106,
            filterHighHz: 0);
    ...
    [Fact]
    public void ApplyRxAudioLeveler_HoldsNativeNr5SpeechValleyFromLive7335Trace()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 5.4,
        ...
        float[] block = new float[1024];
        Array.Fill(block, DbToLinear(-34.4));
        var nr5 = Nr5Diagnostics(
            signalConfidence: 0.295,
            signalProbability: 0.095,
            agcGate: 0.625,
            recoveryDrive: 0.463,
            weakSignalMemory: 0.504,
            outputDbfs: -36.1,
            inputDbfs: -44.6,
            maskSmoothing: 0.332,
            peakEvidence: 0.270,
            outputPeakDbfs: -10.8,
            adjacentNoiseTrust: 0.507,
            adjacentNoiseDrive: 0.449,
            levelDrive: 0.968);
        ...
        DspPipelineService.ApplyRxAudioLeveler(
            ...
            ref state,
            nr5,
            FrontendTopPeak(offsetHz: 9_938, snrDb: 27.6, dbfs: -79.2, confidence: 0.700),
            filterLowHz: -3_106,
            filterHighHz: 0);
    ...
    [Fact]
    public void ApplyRxAudioLeveler_KeepsLowProofNativeNr5ValleyCapped()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 5.4,
        ...
        var nr5 = Nr5Diagnostics(
            signalConfidence: 0.282,
            signalProbability: 0.076,
            agcGate: 0.625,
            recoveryDrive: 0.120,
            weakSignalMemory: 0.140,
            outputDbfs: -36.1,
            inputDbfs: -44.6,
            maskSmoothing: 0.240,
            peakEvidence: 0.030,
        ...
        Assert.True(
            state.DesiredGainDb <= 10.8,
            $"desired={state.DesiredGainDb:F2}, applied={state.AppliedGainDb:F2}, output={state.OutputRmsDbfs:F1}");
        Assert.True(
            state.AppliedGainDb <= 10.8,
            $"desired={state.DesiredGainDb:F2}, applied={state.AppliedGainDb:F2}, output={state.OutputRmsDbfs:F1}");
        Assert.True(state.OutputRmsDbfs <= -23.5, $"output={state.OutputRmsDbfs:F1}");
        Assert.False(state.OutputLimited);
    }
    ...
    [Fact]
    public void ApplyRxAudioLeveler_LiftsHeldPassbandStrongUnderReportedNr5Syllable()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 3.7,
            PauseHoldBlocks = 18,
            Nr5SpeechHoldBlocks = 19
        };
        float[] block = new float[1024];
        Array.Fill(block, DbToLinear(-30.2));
        block[0] = DbToLinear(-27.9);
        var nr5 = Nr5Diagnostics(
            signalConfidence: 0.250,
            signalProbability: 0.016,
            agcGate: 0.586,
            recoveryDrive: 0.102,
            weakSignalMemory: 0.001,
            outputDbfs: -26.1,
        ...
        DspPipelineService.ApplyRxAudioLeveler(
            ...
            ref state,
            nr5,
            FrontendTopPeak(offsetHz: -187, snrDb: 40.7, dbfs: -32.9, confidence: 0.940),
            filterLowHz: -3_106,
            filterHighHz: 0);

        Assert.InRange(state.DesiredGainDb, 9.0, 13.0);
        Assert.Equal(state.DesiredGainDb, state.AppliedGainDb, precision: 6);
    ...
    [Fact]
    public void ApplyRxAudioLeveler_DoesNotLiftHeldUnderReportedNr5SyllableWithoutPassbandProof()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 3.7,
        ...
        Assert.InRange(state.DesiredGainDb, 0.0, 4.0);
        Assert.Equal(state.DesiredGainDb, state.AppliedGainDb, precision: 6);
    ...
    public void ApplyRxAudioLeveler_LiftsHeldPassbandWeakUnderReportedNr5Syllable()
    {
        ...
        float[] block = new float[1024];
        Array.Fill(block, DbToLinear(-41.1));
        block[0] = DbToLinear(-34.4);
        var nr5 = Nr5Diagnostics(
            signalConfidence: 0.289,
            signalProbability: 0.146,
            agcGate: 0.566,
            recoveryDrive: 0.133,
            weakSignalMemory: 0.005,
            outputDbfs: -29.5,
            inputDbfs: -21.8,
            maskSmoothing: 0.386,
            peakEvidence: 0.0,
        ...
        DspPipelineService.ApplyRxAudioLeveler(
            ...
            ref state,
            nr5,
            FrontendTopPeak(offsetHz: -187, snrDb: 45.9, dbfs: -45.0, confidence: 0.940),
            filterLowHz: -3_106,
            filterHighHz: 0);

        Assert.InRange(state.DesiredGainDb, 18.0, 22.5);
        Assert.Equal(state.DesiredGainDb, state.AppliedGainDb, precision: 6);
    ...
    [Fact]
    public void ApplyRxAudioLeveler_CapsPassbandLowProofContinuityPumpFromLive40mTrace()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 32.9,
        ...
        float[] block = new float[1024];
        Array.Fill(block, DbToLinear(-64.2));
        var nr5 = Nr5Diagnostics(
            signalConfidence: 0.320,
            signalProbability: 0.149,
            agcGate: 0.294,
            recoveryDrive: 0.172,
            weakSignalMemory: 0.036,
            outputDbfs: -37.6,
            inputDbfs: -40.2,
            maskSmoothing: 0.0,
            peakEvidence: 0.0,
            outputPeakDbfs: -29.9,
            adjacentNoiseTrust: 0.538,
            adjacentNoiseDrive: 0.494,
            levelDrive: 0.511);
        ...
        DspPipelineService.ApplyRxAudioLeveler(
            ...
            ref state,
            nr5,
            FrontendTopPeak(offsetHz: -1_500, snrDb: 25.6, dbfs: -79.2, confidence: 0.940),
            filterLowHz: -3_106,
            filterHighHz: 0);

        Assert.InRange(state.DesiredGainDb, 23.6, 24.2);
        Assert.Equal(state.DesiredGainDb, state.AppliedGainDb, precision: 6);
        Assert.InRange(state.OutputRmsDbfs, -40.6, -40.0);
        Assert.True(
            state.GainDeltaDb < -7.0,
            $"desired={state.DesiredGainDb:F1} applied={state.AppliedGainDb:F1} delta={state.GainDeltaDb:F1}");
        Assert.False(state.OutputLimited);
    }

    [Fact]
    public void ApplyRxAudioLeveler_DoesNotCapPassbandContinuityWithNativeProof()
    {
        var state = new DspPipelineService.RxAudioLevelerState
        {
            GainDb = 32.9,
        ...
        Array.Fill(block, DbToLinear(-64.2));
        var nr5 = Nr5Diagnostics(
            signalConfidence: 0.340,
            signalProbability: 0.205,
            agcGate: 0.460,
            recoveryDrive: 0.340,
            weakSignalMemory: 0.260,
            outputDbfs: -36.8,
            inputDbfs: -40.2,
            maskSmoothing: 0.260,
            peakEvidence: 0.045,
